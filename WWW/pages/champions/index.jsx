...
const ChampionsPage = (props) => {
	...
	const { darkmode, toggleDarkMode, championNamesAndTypes } = props;
	const processToFilename = (string) => {
		return string.replace(/[^a-zA-Z0-9]/g, "");
	};
	
	const fixedChampionNames = championNamesAndTypes.map((champion) => ({...champion, imgName: processToFilename(champion.name)}));
	const [ championList ] = useState(fixedChampionNames);
	const [ championTypes ] = useState(["Assassin", "Fighter", "Mage", "Marksman", "Support", "Tank"]);
	const [ filter, setFilter ] = useState("");
	const [ selectedTypes, setSelectedTypes ] = useState([]);

	const handleCheck = (e) => {
		const item = e.target.value;
		if (selectedTypes.some(type => type === item)) {
					...
					{championList
						...
						.filter(item => {
							...
							item.roles.forEach(role => {
								if(selectedTypes.some(selectedRole => selectedRole == role)) {
									condition = true;
								}
								else {
									condition = false;
...
export async function getStaticProps() {
	const req = await fetch("https://localhost:5001/api/Champion/NamesAndTypes", {
		method: "GET",
		mode: "cors",
		cache: "default",
		headers: {
	...
	const championNamesAndTypes = await req.json();
	return {
		props: {
			championNamesAndTypes
		}
	};
}